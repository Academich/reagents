...
class Augmenter:
    ...
    The class has one method, `augment`, which is overridden by child classes.
...
class SMILESAugmenter(MolAugmenter):
    ...
    def augment_smiles(self, smiles: Iterable[str]) -> List[str]:
        ...
        for smi in smiles:
            ...
            mols = list(map(Chem.MolFromSmiles, smi.split(".")))
            for _ in range(3):
                try:
                    ...
                    print("Augmentation failed for {} with error: {}".format(smi, e))
            else:
                smiles_aug.append(smi)
                print("Augmentation failed three times for {}, returning unaugmented original".format(smi))
